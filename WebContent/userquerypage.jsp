			...
			var obj = document.getElementsByName("type");
   			for(var i=0; i<obj.length; i ++){
        		if(obj[i].checked){
            		...
            		form.method="post";
            		form.target="_blank";
					form.action="v1/container/getJson/aflQuery";
					form.submit();
            		}else if(obj[i].value=="csv"){
            		//alert("csv");
            		form.method="post";
            		form.target="_blank";
					form.action="v1/container/getCsv/aflQuery";
					form.submit();
            		}else if(obj[i].value=="html"){
            		//alert("html");
            		form.method="post";
            		form.target="_self";
					form.action="v1/user/aflQuery";
					form.submit();
            		}
        		}
   			 }
			...
			var obj = document.getElementsByName("type");
   			for(var i=0; i<obj.length; i ++){
        		if(obj[i].checked){
            		...
            		form.method="post";
            		form.target="_blank";
					form.action="v1/container/getJson/aqlQuery";
					form.submit();
            		}else if(obj[i].value=="csv"){
            		//alert("csv");
            		form.method="post";
					...
					form.action="v1/container/getCsv/aqlQuery";
					form.submit();
            		}else if(obj[i].value=="html"){
            		//alert("html");
            		form.method="post";
            		form.target="_self";
					form.action="v1/user/aqlQuery";
					form.submit();
            		}
        		}
   			 }
				...
				<div class="span12">
                    	<form  method="POST" action="" id="query" target=""><!--role="form"-->
 							<div> <!--class="form-group" -->   
								...
								<p class="lead"> You can search the  database with a <em>query</em>. A query is a request for information. 
								...
								<c:if test="${sessionScope.user ne null}">
									<a href="<c:url value='/findQuery'/>" class="btn btn-default">Refresh My Statement</a>
								</c:if>
								<c:if test="${not empty requestScope.rows and requestScope.rows  ne 0}">
								<div class="alert alert-info" role="alert">Total ${requestScope.rows} records, Size: ${requestScope.dataSize}, Takes ${requestScope.time} ms</div>
								</c:if>
								<c:if test="${not empty requestScope.addInfo}">
								<div class="alert alert-info" role="alert">${requestScope.addInfo }</div>
								</c:if>
								<!-- 
				...
				<div id="myTabContent" class="tab-content">
					<div class="tab-pane fade in active" id="qs">
					    ...
					    <%	
							...
							if (pageCount != 0){						
							%>
							No.<%=showPage %>（total:<%=pageCount %> pages）
 							<a href="QuerypageSer?showPage=1">First</a>
				   ...
				   <div class="tab-pane fade" id="history">
	 				...
	 				<table class="table table-bordered table-striped">
	  					...
	  					<c:if test="${sessionScope.user ne null}">
	  					 <td><a href="<c:url value='/addQuery?qs=${item.value}' />" id="addStmt" onclick="addStmtClick()" class="btn btn-primary btn-small">添加为常用语句</a></td></tr>
						</c:if>
						</c:forEach>
	 				</table>
				   </div>
				   ...
				   <!-- 显示已注册用户的常用查询语句 -->
					<table class="table table-bordered table-striped">
	  					<tr>
	  						<td>My Statement</td><td>Time</td><td>Operation</td>
	  					</tr>
	  					<c:if test="${sessionScope.userQuery ne null}">
	  						<c:forEach items="${sessionScope.userQuery}" var="uq">
	  						<tr><td>${uq.queryString }</td> <td>${uq.queryTime}</td><td><a href="<c:url value='/deleteQuery?qid=${uq.queryId}'/>" class="btn btn-danger btn-small" id="delStmt" onclick="delStmt">删除</a></td></tr>
								</c:forEach>
						</c:if>
  					</table>
				   </div>
